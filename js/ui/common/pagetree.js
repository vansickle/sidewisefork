...
function onPageRowFormatTooltip(evt) {
  ...
  if (loggingEnabled) {
    ...
    url += '<br/><br/>Id: ' + page.id + '<br/>History length: ' + page.historylength + '<br/>Referrer: ' + (page.referrer || "''") + '<br/>Chrome ID: ' + (page.chromeId || "''") + '<br/>WinId/index: ' + page.windowId + '/' + page.index;
  }
...
function onWindowRowFormatTitle(row, itemTextElem) {
  ...
  text = (label ? '' : text) + ' (' + childCount + ' ' + getMessage(childCount == 1 ? 'text_page' : 'text_pages') + ')';
...
function onWindowRowFormatTooltip(evt) {
  ...
  var body = childCount + ' ' + (incognito ? 'incognito' + ' ' : '') + (childCount == 1 ? getMessage('text_page') : getMessage('text_pages'));
